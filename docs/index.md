...
- [database.md](database.md) · [🇷🇺](database.ru.md) — the schema as it exists in `scripts/*.sql`
  (`papers`, `chunks`, `halfvec`, `text_search`, HNSW), the lifecycle backfill, and the migration
  file order.
- [enrichment.md](enrichment.md) · [🇷🇺](enrichment.ru.md) — `summary_en` / `summary_ru` / `tags`
  generated by Gemini, decoupled from `ingest_status`, not yet used in retrieval.
- [bot-commands.md](bot-commands.md) · [🇷🇺](bot-commands.ru.md) — operator reference for
  `/start /search /ask /list /enrich /reindex` and the inline ingest buttons.
- [configuration.md](configuration.md) · [🇷🇺](configuration.ru.md) — environment variables, the
  Docker Compose `db`/`app` services, dependencies, and the manual schema-init caveat.
...
The as-built concept + diagram set is complete. Remaining bundle work:

- Per-file OKF frontmatter for `plan/01`–`plan/05` (only [plan/README.md](plan/README.md) is
  marked so far).
- Human review: promote agent-drafted docs from `status: draft` to `verified` after checking.
